...
exports.setApp = function (app, client) {
    ...
    app.post('/api/mobile/inbox/apply', cookieJwtAuth, async (req, res, next) => {
        ...
        try {
            ...
            if(resultFindUser == null || resultFindUser == undefined) return res.status(404).json({ newToken: req.token, error:"User not found" });
            resultsFindInbox = await db.collection('Inbox').find({ projectID: projectNid, userID: userNid}).toArray();
            if(resultsFindInbox.length > 0) return res.status(403).json({ newToken: req.token, error:"User already applied to this project" });
            resultFindProjects = await db.collection('Projects').findOne({ _id: projectNid });
            if(resultFindProjects == null || resultFindProjects == undefined) return res.status(404).json({ newToken: req.token, error: "project with given projectId not found"});
            var isUserApart = false;
            resultFindProjects.teamMembers.forEach((x, i) => {
                if(x.username == resultFindUser.username) isUserApart = true;
            });
            if(isUserApart) return res.status(401).json({ newToken: req.token, error:"User is already a member of this project"});
            var roleFound = false;
            resultFindProjects.roles.forEach((x, i) => {
                if(x.role == role) roleFound = true;
            });
            if(!roleFound) return res.status(404).json({ newToken: req.token, error: "given role not found in the project the given projectId"});
            const newInbox = {
            ...
            var ret = { newToken: req.token, error : "" };
    ...
    app.post('/api/mobile/inbox/accept_member', cookieJwtAuth, async (req, res, next) => {
        ...
        try {
            ...
            if(resultFindUser == null || resultFindUser == undefined) {
                return res.status(404).json({ newToken: req.token, error: "User not found"});
            }
            ...
            if(resultFind != null || resultFind != undefined){
                ...
                if(roleFound){
                    ...
                    return res.status(404).json({ newToken: req.token, error: "Role not found"});
                }
            }else{
                return res.status(404).json({ newToken: req.token, error: "Project not found"});
            }
        } catch (e) {
            error = e.toString;
            var ret = { newToken: req.token, error: error };
    ...
    app.get('/api/mobile/inbox/:projectId', cookieJwtAuth, async (req, res, next) => {
        ...
        if(resultFindProject == null || resultFindProject == undefined) return res.status(400).json({ newToken: req.token, error: "Project not found"});
        ...
        if(results.length > 0){
            ...
            var ret = { projectTitle: resultFindProject.title, appliedUsers, newToken: req.token, error: error};
            return res.status(200).json(ret);
        }else{
            var appliedUsers = [];
            var ret = { projectTitle: resultFindProject.title, appliedUsers, newToken: req.token, error: error};
